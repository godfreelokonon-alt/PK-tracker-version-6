...
// Multi-listener : tableau de callbacks — tracker.js ET ui.js peuvent s'abonner sans conflit
const _stepCallbacks    = [];
...
function start() {
  ...
  _accelListener = (e) => {
    ...
    if (_highpass > STEP_THRESHOLD && (now - _lastStepTs) > MIN_STEP_INTERVAL_MS) {
      _stepCount++;
      _lastStepTs = now;
      _stepCallbacks.forEach(cb => { try { cb(_stepCount, now); } catch(e) {} });
    }
  };
...
// onStep : addListener — ajoute sans écraser les callbacks existants
function onStep(cb) {
  if (typeof cb === 'function' && !_stepCallbacks.includes(cb)) _stepCallbacks.push(cb);
}
function offStep(cb) {
  const i = _stepCallbacks.indexOf(cb);
  if (i > -1) _stepCallbacks.splice(i, 1);
}
...
window.PKT_MOTION = {
  ...
  onStep, offStep, onHeading, resetSteps, getSteps, setSteps,
